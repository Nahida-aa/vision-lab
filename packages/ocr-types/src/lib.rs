//! Pure data types + utility functions for the OCR pipeline.
//!
//! This crate has **no heavy dependencies** (no ort, opencv, ffmpeg-next).
//! It provides the shared type definitions and pure algorithm functions
//! used by both `subtitle-ocr` (engine) and `subtitle-ocr-post` (algorithms),
//! as well as downstream consumers like LocalDub.
